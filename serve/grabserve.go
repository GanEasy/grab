...
`)
	...
	api.GET("/list/:url", func(c echo.Context) error {
		...
		if drive == `qidian` || drive == `zongheng` || drive == `jx` || drive == `paoshu8` || drive == `shuge` || drive == `qkshu6` || drive == `xs280` || drive == `hongxiu` || drive == `xxsy` || drive == `biqugeinfo` || drive == `mcmssc` || drive == `17k` || drive == `xbiquge` || drive == `luoqiu` || drive == `booktxt` || drive == `bxwx` || drive == `uxiaoshuo` || drive == `biquyun` || drive == `soe8` {
			go a.SyncPosts(list, 1)
		} else if drive == `manhwa` || drive == `kanmeizi` || drive == `haimaoba` || drive == `hanmanwo` || drive == `hanmanku` || drive == `ssmh` || drive == `fuman` || drive == `aimeizi5` {
			go a.SyncPosts(list, 2)
		}
